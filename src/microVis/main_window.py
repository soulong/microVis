...
class MainWindow(QMainWindow):
        ...
        if object_table and self._overlay_col:
            try:
                ...
                if odf is not None and "label" in odf.columns and self._overlay_col in odf.columns:
                    # Scope per-image to avoid label ID collisions across fields/stacks/tps
                    group_cols = ["well", "field", "stack", "timepoint"]
                    has_image_cols = all(c in odf.columns for c in group_cols)
                    if has_image_cols:
                        for (well, field, stack, tp), gdf in odf.groupby(group_cols):
                            per_object_values.setdefault(well, {})[(field, stack, tp)] = dict(
    ...
    def _on_export_clicked(self) -> None:
        ...
        channel_scope = ic.get_export_channel_filter()
        channel_names = self._dm.channels
        if channel_scope == "Checked channels":
            ch_config = ic.get_channel_config()
            channel_names = [ch for ch in channel_names if ch_config.get(ch, {}).get("enabled", False)]
